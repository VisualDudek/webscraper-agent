...
import json, re, sys
import requests, cloudscraper, feedparser
from bs4 import BeautifulSoup      # docs: https://www.crummy.com/software/BeautifulSoup/bs4/doc/
...
def session() -> requests.Session:
    ...
    s.headers.update({"User-Agent": UA, "Accept-Language": "pl-PL,pl;q=0.9,en-US;q=0.8"})
    return s
    ...
    return [
        {
            ...
            "url":     item["link"],
            "excerpt": BeautifulSoup(item["excerpt"]["rendered"], "html.parser").get_text(" ", strip=True),
            "date":    item["date"],
        } for item in r.json()
    ]
    ...
    return [
        {
            "title":   entry.title,
            "url":     entry.link,
            "excerpt": BeautifulSoup(entry.summary, "html.parser").get_text(" ", strip=True)
                       if "summary" in entry else "",
            "date":    datetime(*entry.published_parsed[:6]).isoformat()
                       if "published_parsed" in entry else "",
...
def html_scrape(s: requests.Session):
    ...
    for art in soup.select(ARTICLE_SEL):
        ...
        if not a:
            continue
        excerpt_tag = (
            art.find("div", class_=re.compile("excerpt|summary"))
            or art.find("p")
        )
        results.append(
            {
